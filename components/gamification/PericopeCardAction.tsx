...
// The card's one action: a "Locked" preview button, the active lesson's own "Today's
// Verse(s)" text + "Learn" button, or a plain single-line button (Review, a capstone day's
// own name) for every other case. Renders nothing for a spillover-only card — its own
// independent action would be redundant right above the section that actually owns the
// lesson's one "Learn" button (see PericopeCard.tsx).
export function PericopeCardAction({ isSpilloverOnly, status, isTodaysLesson, todaysVerses, actionLabel, onClick }: PericopeCardActionProps) {
